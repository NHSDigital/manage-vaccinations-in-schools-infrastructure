...
1. Deploy the migration-ready infrastructure configuration. This can be done either:
   ...
   1. Running the [deploy-infrastructure.yml](../../../../.github/workflows/deploy-infrastructure.yml)
      workflow on a prepared branch/tag, using the already deployed docker image tag to not change the code of the
      running application.
1. Take snapshot of source DB
1. DB failover (this ensures write instance has correct DB parameters)
   1. The read instance will need to be rebooted first if it was not created in step 1.
1. Shell into `mavis-<ENV>-prepare_new_db` service and run
   ...
   1. Run `bin/rails dbconsole` and execute the following commands to prepare the target DB:
   ```postgresql
...
1. Wait until full load is done and migration is in replication mode
1. Update the sequences on target DB
   1. Run [get_alter_sequence_statements.sql](resources/get_alter_sequence_statements.sql) script on source DB
   1. This will generate a set of `ALTER SEQUENCE` SQL commands that need to be executed on the target DB.
      The script will output the commands to the console, which can be copied and executed on the target DB.
   1. The script is configured to add a buffer of 10000 to the current sequence values to handle any potential data
      inserts during switchover. Modify the value of the `sequence_buffer` variable in the script if needed.
1. On target DB execute the ALTER sequence SQL commands
1. Validate new DB content looks good
1. Take snapshot of source DB
1. Deploy infrastructure configuration pointing ECS services to new database.
   1. This is done by running the [deploy-infrastructure.yml](../../../../.github/workflows/deploy-infrastructure.yml)
      ...
      running application.
   1. Note that this also changes the DB session_replication_role to the default "origin"), which is crucial to ensure
      data consistency on the target DB after the migration.
1. Validate that the target DB has `session_replication_role` set to `origin` in the DB cluster parameter group and is in sync0.
...
1. Stop good-job-service (by updating service and setting descried count to 0)
1. Execute CodeDeploy deployment of web-service (after deploy it will point against new service)
1. Stop DMS migration task
1. Execute ECS deployment of good-job-service
...
1. Destroy no-longer needed resources
   1. This is done by running the [deploy-infrastructure.yml](../../../../.github/workflows/deploy-infrastructure.yml)
      workflow on a prepared branch/tag. Use the already deployed docker image tag to not change the code of the
      running application.
   1. This should remove the migration-specific sources and apply post-migration changes like password rotation
      frequency.
   1. The old source DB cluster and all its references should also be destroyed by this deployment.
