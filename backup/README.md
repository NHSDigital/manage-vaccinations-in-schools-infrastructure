...
1. Set up the **destination** account by running `terraform apply` in the `destination` directory. Pass the account_id of the source account as input.
   It returns the ARN of the destination vault that is created.
1. Put the ARN of the destination vault in the \*.tfvars file in the `source` directory.
1. Create an AWS policy based on the `aws-backup-policy.json` file.
...
1. Set up the **source** account by running the `deploy-backup-infrastructure.yml` GitHub Action workflow.
...
1. Set a new DB cluster identifier, e.g. `mavis-$ENV-restored` and click on "Restore".
1. Wait for the restored DB cluster to be complete.
1. Update the DB credentials:
...
1. Refresh the terraform state of the app by running `terraform refresh`.
1. Import the new DB cluster into the terraform state
...
1. Restart any running ECS tasks.
